...
public class ControladorLogin {
	...
	public ModelAndView validarLogin(@ModelAttribute("datosLogin") DatosLogin datosLogin, HttpServletRequest request) {
		...
		if (usuarioBuscado != null) {
			...
			if(usuarioBuscado.getRol().equals("medico")){
				return new ModelAndView("redirect:/home-medico");
			}else if (usuarioBuscado.getRol().equals("paciente")){
	...
	public ModelAndView irAHomeMedico(HttpServletRequest request) {
		if (request.getSession().getAttribute("ROL").equals("medico")){
			ModelMap modelo = new ModelMap();
			Usuario usuario = servicioLogin.consultarUsuario(request.getSession().getAttribute("ID"));
			modelo.put("usuario", usuario);
			return new ModelAndView("home-medico", modelo);
		}else {
	...
	public ModelAndView irAHomePaciente(HttpServletRequest request) {
		if (request.getSession().getAttribute("ROL").equals("paciente")){
